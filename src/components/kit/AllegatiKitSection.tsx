...
import {
  BUCKET_KIT, CATEGORIE_KIT, CATEGORIE_KIT_LABEL,
  type AllegatoKit, type CategoriaAllegatoKit,
  deleteAllegatoKit, fetchAllegatiKit,
  getSignedUrlKit, uploadAllegatoKit,
} from "@/lib/allegati-kit-api";
import { formatBytes } from "@/lib/allegati-articolo-api";
import { InviaEmailDialog } from "@/components/preventivi/InviaEmailDialog";

export type KitEmailContext = {
  nome?: string | null;
};
...
async function downloadAllegato(a: AllegatoKit) {
  const { data, error } = await supabase.storage.from(BUCKET_KIT).download(a.storage_path);
...
async function printAllegato(a: AllegatoKit) {
  const url = await getSignedUrlKit(a.storage_path, 300);
...
export function AllegatiKitSection({
  kitId,
  emailContext,
}: {
  kitId: string;
  emailContext?: KitEmailContext;
}) {
  const qc = useQueryClient();
  const [uploadOpen, setUploadOpen] = useState(false);
  const [preview, setPreview] = useState<AllegatoKit | null>(null);
  const [emailTarget, setEmailTarget] = useState<{ allegato: AllegatoKit; blob: Blob } | null>(null);

  const { data: allegati = [], isLoading } = useQuery({
    queryKey: ["allegati_kit", kitId],
    queryFn: () => fetchAllegatiKit(kitId),
  });

  const invalidate = () => qc.invalidateQueries({ queryKey: ["allegati_kit", kitId] });

  const grouped = useMemo(() => {
    const m = new Map<CategoriaAllegatoKit, AllegatoKit[]>();
    for (const a of allegati) {
      const list = m.get(a.categoria) ?? [];
      list.push(a);
      m.set(a.categoria, list);
    }
    return CATEGORIE_KIT.filter((c) => m.has(c)).map((c) => ({ cat: c, items: m.get(c)! }));
  }, [allegati]);

  const del = useMutation({
    mutationFn: (a: AllegatoKit) => deleteAllegatoKit(a),
    onSuccess: () => { toast.success("Allegato eliminato"); invalidate(); },
    onError: (e: unknown) => toast.error((e as Error).message),
  });

  async function handleDownload(a: AllegatoKit) {
    try { await downloadAllegato(a); } catch (e) { toast.error((e as Error).message); }
  }
  async function handlePrint(a: AllegatoKit) {
    try { await printAllegato(a); } catch (e) { toast.error((e as Error).message); }
  }
  async function handleEmail(a: AllegatoKit) {
    try {
      const { data, error } = await supabase.storage.from(BUCKET_KIT).download(a.storage_path);
      if (error) throw error;
          ...
          {grouped.map(({ cat, items }) => (
            <div key={cat} className="space-y-1.5">
              <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                {CATEGORIE_KIT_LABEL[cat]} ({items.length})
              </div>
...
function PreviewDialog({
  ...
  allegato: AllegatoKit | null;
  onOpenChange: (v: boolean) => void;
  onDownload: (a: AllegatoKit) => void;
  onPrint: (a: AllegatoKit) => void;
}) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data, error } = await supabase.storage.from(BUCKET_KIT).download(allegato.storage_path);
        if (error) throw error;
...
function UploadDialog({
  kitId, open, onOpenChange, onDone,
}: {
  kitId: string;
  open: boolean;
  onOpenChange: (v: boolean) => void;
  ...
  const [categoria, setCategoria] = useState<CategoriaAllegatoKit>("altro");

  const up = useMutation({
    mutationFn: () => {
      if (!file) throw new Error("Seleziona un file");
      return uploadAllegatoKit({ kitId, file, categoria });
    },
    onSuccess: () => {
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
          ...
          <div className="grid gap-1.5">
            ...
            <Select value={categoria} onValueChange={(v) => setCategoria(v as CategoriaAllegatoKit)}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {CATEGORIE_KIT.map((c) => (
                  <SelectItem key={c} value={c}>{CATEGORIE_KIT_LABEL[c]}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
